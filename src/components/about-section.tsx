...
export function AboutSection() {
  return (
    ...
    >
      <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_1fr] items-stretch">
        {/* Left Column: Text & Focus Area Cards */}
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {focusAreas.map((area, index) => (
              <motion.div
                ...
                className="rounded-2xl border border-white/70 bg-white/75 p-5 shadow-card backdrop-blur-xl cursor-pointer"
              >
        ...
        {/* Right Column: Standalone Workflow SVG */}
        <div className="relative min-h-[320px] lg:min-h-full">
          <motion.div 
            whileHover={{ y: -4, scale: 1.01 }}
            transition={{ duration: 0.3, ease: "easeOut" }}
            className="relative w-full h-full min-h-[320px] overflow-hidden rounded-2xl border border-white/70 bg-white/70 shadow-card backdrop-blur-xl group cursor-pointer"
          >
            <Image
